...
function UASORT_EDUCATION_CMP($a,$b){
	return strcasecmp($a->getData("title",""),$b->getData("title",""));
}
...
class Education {
    ...
    public function __construct($path=null){
        ...
        if($path===null || $path===false || !is_string($path) || strlen($path)==0){
            $this->path="${LIB_PATH}/../DB/education" ;
        }else
            $this->path = $path;
    }
    ...
    public function load(){
        ...
        while (($file = readdir($dh)) !== false) {
            ...
            $c=new EducationResource($this,$file);
            if($c->loadData()){            
	...
	public function getResourcesFiltered($tags,$types,$media,$search,$start,$num){
		$resources=$this->getResources();
		
		uasort($resources, "UASORT_EDUCATION_CMP");
		
		if(is_array($tags) && count($tags)>0){
			foreach($resources as $k=>$res)
				if(!$res->hasAnyTags($tags))unset($resources[$k]);
		}
